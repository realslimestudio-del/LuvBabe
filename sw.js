...
async function getStaffs() {
  try {
    ...
    return {missing:false, staffs:null};
  }
}
...
async function passThroughWithoutStaff(req, payload) {
  const clean = {...payload};
  delete clean.staffs;
  ...
  if (event.request.method==='GET' && decodedPath.endsWith('/index (1).html')) {
    event.respondWith((async()=>{
      ...
      try{
        ...
        const fixed=text.replace('staffs:S.staffs?.length?S.staffs:P.staffs','staffs:Array.isArray(S.staffs)?S.staffs:P.staffs');
    ...
    event.respondWith((async()=>{
      ...
      if(method==='GET'){
        try{
          ...
          const remote=await getStaffs();
          if(Array.isArray(remote.staffs)){
            data.staffs=remote.staffs;
            lastStaffSnapshot=remote.staffs.map(x=>({...x}));
            lastStaffWriteAt=Date.now();
          }else if(remote.missing){
            const initial=Array.isArray(data.staffs)?data.staffs:[];
            await saveStaffs(initial);
            data.staffs=initial;
          }else if(Array.isArray(lastStaffSnapshot)&&Date.now()-lastStaffWriteAt<120000){
      ...
      if(method==='POST'||method==='PUT'){
        ...
        if(payload&&Array.isArray(payload.staffs)){
          if(payload.staffs.length>0&&Date.now()<clearLockUntil){
            return passThroughWithoutStaff(req,payload);
          }
